...
class handler(BaseHTTPRequestHandler):
    ...
    def process_api(self, method):
        ...
        if action == "save_memory":
            ...
            if row_id:
                db_req = urllib.request.Request(f"{supabase_url}/rest/v1/memories?id=eq.{row_id}", data=json.dumps(db_payload).encode('utf-8'), method="PATCH")
            else:
                db_req = urllib.request.Request(f"{supabase_url}/rest/v1/memories", data=json.dumps(db_payload).encode('utf-8'), method="POST")
