...
function renderRightPanel() {
    ...
    if (!selectedPref) {
        ...
        html += `<h2 style="margin:0; font-size:1.4rem;">あしあと</h2>`;
        ...
        html += `<button onclick="selectedPref=null; renderRightPanel();" style="background:white;">← 戻る</button>`;
        html += `<button onclick="closePanel()" style="border:none; background:none; font-size:24px; padding:0;">✕</button>`;
        html += `</div>`;
...
async function saveMemoryData() {
    ...
    const payload = { action: "save_memory", prefecture: selectedPref, date: '', photos: base64Photos };
