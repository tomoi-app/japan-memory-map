...
function switchToSignup() {
    ...
    // ★修正：チェックボックスの状態を見てボタンの有効/無効を判定する
    const agreeCheck = document.getElementById('privacy-agree-check');
    document.getElementById('auth-submit-btn').disabled = !(agreeCheck && agreeCheck.checked);
...
async function performQueuedSave(targetPref, targetEntryId, fromVal, toVal, memoValue, files) {
    ...
    const statusEl = document.getElementById('autosave-status');
    const isHeavyTask = files && files.length > 0;
    ...
    try {
        ...
        let successfullyAddedIds = []; // ★無事に追加できた写真のIDを記録しておく箱
        ...
        if (isHeavyTask) {
            ...
            updateMapColors();
            updateCounter();
            updateSaveProgress(files.length, files.length, `${totalSaved}枚保存完了！`);
        ...
        if (isGuestMode) {
            ...
            if (res.ok) {
                ...
                if (isHeavyTask) {
                    updateSaveProgress(files.length, files.length, `${files.length}枚保存完了！`);
                    setTimeout(() => { hideSaveProgress(); }, 2500);
                }
            } else {
