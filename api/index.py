...
class handler(BaseHTTPRequestHandler):
    ...
    def process_api(self, method):
        ...
        if method == "GET":
            ...
            if share_token:
                try:
                    ...
                    response_data = {"memories": records, "settings": {"show_thumb": show_thumb == "1", "show_date": show_date == "1"}}
                    self.send_response(200)
                    self.send_header('Content-type', 'application/json; charset=utf-8')
                    self.send_header('Access-Control-Allow-Origin', '*')
                    self.end_headers()
        ...
        try:
            ...
            if not current_user_id: raise Exception("user id empty")
        ...
        if content_length == 0: raise Exception("Request body is empty.")
        ...
        if action == "generate_share_link":
            ...
            expires_at = int(time.time()) + (3153600000 if expires_in == -1 else expires_in)
            ...
            if entry_id:
                db_payload = {"date": date_str, "photo_urls": json.dumps(final_photo_urls), "memo": memo_str}
                ...
                with urllib.request.urlopen(db_req, timeout=15): pass
                data = json.dumps([{"id": entry_id}]).encode('utf-8')
            else:
                db_payload = {
                    "prefecture": pref, "date": date_str, "photo_urls": json.dumps(final_photo_urls), "memo": memo_str,
                    "title": "Memory", "lat": 0.0, "lng": 0.0, "user_id": current_user_id, "is_home": False
                }
                db_req = urllib.request.Request(
                    f"{supabase_url}/rest/v1/memories",
                    data=json.dumps(db_payload).encode('utf-8'), method="POST"
                )
                db_req.add_header("apikey", supabase_key)
                db_req.add_header("Authorization", f"Bearer {user_token}")
                db_req.add_header("Content-Type", "application/json")
            ...
            if entry_id:
                rpc_payload = {"p_entry_id": entry_id, "p_new_urls": new_urls, "p_date": date_str, "p_memo": memo_str}
                rpc_req = urllib.request.Request(
                    f"{supabase_url}/rest/v1/rpc/append_photo_urls_v2",
                    data=json.dumps(rpc_payload).encode('utf-8'), method="POST"
                )
                ...
                with urllib.request.urlopen(rpc_req, timeout=15): pass
                data = json.dumps([{"id": entry_id}]).encode('utf-8')
            else:
                db_payload = {
                    "prefecture": pref, "date": date_str, "photo_urls": json.dumps(new_urls), "memo": memo_str,
                    "title": "Memory", "lat": 0.0, "lng": 0.0, "user_id": current_user_id, "is_home": False
                }
                db_req = urllib.request.Request(
                    f"{supabase_url}/rest/v1/memories",
            ...
            reset_req = urllib.request.Request(
                ...
                data=json.dumps({"is_home": False}).encode('utf-8'), method="PATCH"
            )
            ...
            for pref in home_prefs:
                ...
                if rows:
                    ...
                    up_req = urllib.request.Request(
                        ...
                        data=json.dumps({"is_home": True}).encode('utf-8'), method="PATCH"
                    )
                else:
                    up_req = urllib.request.Request(
                        f"{supabase_url}/rest/v1/memories",
                        data=json.dumps({"prefecture": pref, "date": "", "photo_urls": "[]", "title": "Memory", "lat": 0.0, "lng": 0.0, "user_id": current_user_id, "is_home": True}).encode('utf-8'), method="POST"
                    )
            ...
            def send_sendgrid(to_addr, subject, html_body):
                sg_payload = {"personalizations": [{"to": [{"email": to_addr}]}], "from": {"email": from_email, "name": "あしあと"}, "subject": subject, "content": [{"type": "text/html", "value": html_body}]}
                sg_req = urllib.request.Request("https://api.sendgrid.com/v3/mail/send", data=json.dumps(sg_payload).encode("utf-8"), method="POST")
                sg_req.add_header("Authorization", f"Bearer {sendgrid_api_key}")
                sg_req.add_header("Content-Type", "application/json")
                with urllib.request.urlopen(sg_req, timeout=10): pass

            reply_label = ("はい（" + reply_email + "）") if want_reply else "いいえ"
            admin_html = f"<h2>あしあと お問い合わせ</h2><p><b>名前：</b>{name}</p><p><b>アカウント：</b>{user_email}</p><p><b>返信希望：</b>{reply_label}</p><hr><p><b>内容：</b></p><p style='white-space:pre-wrap;'>{body}</p>"
            send_sendgrid(from_email, f"【あしあと】お問い合わせ：{name}", admin_html)

            if want_reply and reply_email:
                auto_html = "<p>この度は【あしあと】をご利用いただき、誠にありがとうございます。</p><p>お問い合わせを受け付けました。<br>返信まで少々お待ちください。</p><br><p style='color:#888; font-size:0.9em;'>※このメールは自動送信されています。</p>"
            ...
            del_user_req = urllib.request.Request(f"{supabase_url}/auth/v1/admin/users/{current_user_id}", method="DELETE")
